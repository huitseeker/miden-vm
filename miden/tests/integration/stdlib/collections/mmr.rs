...
fn test_mmr_get_single_peak() -> Result<(), MerkleError> {
    ...
    for pos in 0..(leaves.len() as u64) {
        ...
        let stack: Vec<u64> = leaf
            .iter()
            .map(StarkField::as_int)
            .rev()
            .chain(merkle_root.iter().map(StarkField::as_int).rev())
            .collect();
...
fn test_mmr_get_two_peaks() -> Result<(), MerkleError> {
    ...
    let examples = [
        // root, absolute_pos, leaf
        (merkle_root1, 0, merkle_store.get_node(merkle_root1, NodeIndex::new(3u8, 0u64))?),
        (merkle_root1, 1, merkle_store.get_node(merkle_root1, NodeIndex::new(3u8, 1u64))?),
        (merkle_root1, 2, merkle_store.get_node(merkle_root1, NodeIndex::new(3u8, 2u64))?),
        (merkle_root1, 3, merkle_store.get_node(merkle_root1, NodeIndex::new(3u8, 3u64))?),
        (merkle_root1, 7, merkle_store.get_node(merkle_root1, NodeIndex::new(3u8, 7u64))?),
        (merkle_root2, 8, merkle_store.get_node(merkle_root2, NodeIndex::new(1u8, 0u64))?),
        (merkle_root2, 9, merkle_store.get_node(merkle_root2, NodeIndex::new(1u8, 1u64))?),
    ];

    for (root, absolute_pos, leaf) in examples {
        let source = format!(
            "use.std::collections::mmr
        ...
        let stack: Vec<u64> = leaf
            ...
            .map(StarkField::as_int)
            .rev()
            .chain(root.iter().map(StarkField::as_int).rev())
            .collect();
        test.expect_stack(&stack);
    }
...
fn test_mmr_tree_with_one_element() -> Result<(), MerkleError> {
    ...
    let stack: Vec<u64> = merkle_root3
        .iter()
        .map(StarkField::as_int)
        .rev()
        .chain(merkle_root3.iter().map(StarkField::as_int).rev())
        .collect();
