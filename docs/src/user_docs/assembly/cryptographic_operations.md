...
| hperm  <br> - *(1 cycle)*      | [C, B, A, ...]  | [C', B', A', ...] | $\{A', B', C'\} \leftarrow permute(A, B, C)$ <br> Performs a Rescue Prime Optimized permutation on the top 12 elements of the operand stack, where the top 8 elements are the rate (words C and B), the deepest 4 elements the capacity (word A), the output is the middle elements 4..8 (word B'). |
