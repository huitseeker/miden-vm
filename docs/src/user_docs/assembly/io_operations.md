...
| adv_pipe <br> - *(2 cycles)*     | [S2, S1, A, a, ... ] | [C', B', A', a', ... ] | $[A', B', C'] \leftarrow permute(A, stack.pop(4), stack.pop(4))$ <br> $a' \leftarrow a + 2$ <br> Pops the next two words (8 elements) from the advice stack, inserts them into memory at address $a$ sequentially. Subsequently, performs a Rescue Prime Optimized permutation using the words from the advice stack as the rate state of the permutation, and word A from the operand stack as the capacity. At the end of the operation, the address $a$ is incremented by $2$, the top of operand stack is set with the RPO hasher state, with the RPO output being the middle word B'. <br> Fails if the advice stack has fewer than $8$ values. |
...
| mem_stream <br> - *(2 cycles)* | [S2, S1, A, a, ...] | [C', B', A', a', ...] | $[A', B', C'] \leftarrow permute(A, mem[a], mem[a+1])$ <br> $a' \leftarrow a + 2$ <br> Read two sequential words (8 elements) from memory starting at the address $a$, performs a Rescue Prime Optimized permutation using the memory words as the rate portion of the permutation, and the word A from the operand stack as the capacity. At the end of the operation the address $a$ is incremented by $2$, the top of the operand stack is set with the RPO hasher state, with the ouptut being the middle word B'. |
