...
fn test_set_existing_element() {
    ...
    assert_eq!(matrix.set(0, 0, 5).unwrap(), Some(1));
    assert_eq!(matrix.get(0, 0), 5);
    assert_eq!(matrix.set(1, 1, 6).unwrap(), Some(4));
...
fn test_set_to_zero() {
    ...
    assert_eq!(matrix.set(0, 1, 0).unwrap(), Some(2));
    ...
    assert_eq!(matrix.data, vec![1, 3, 4]);
...
fn test_insert() {
    ...
    // the indptr should be updated correctly
...
fn test_sparse_matrix_large() {
    ...
    assert_eq!(elements[1], 2); // row 0, col 2
    assert_eq!(elements[25], 0); // row 2, col 5 (should be 0)
    assert_eq!(elements[30], 3); // row 3, col 3
