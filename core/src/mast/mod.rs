...
pub struct MastForest {
    ...
    block_decorators: BTreeMap<MastNodeId, Arc<Vec<(usize, DecoratorId)>>>,
...
impl MastForest {
    ...
    pub fn add_block(
        ...
        let adjusted_decorators =
            Arc::new(BasicBlockNode::adjust_decorators(decorators, block.op_batches()));
        let new_node_id = self.add_node(block)?;
        // Always link the decorators to the block, even if empty
        if let Some(MastNode::Block(block)) = self.nodes.get(new_node_id.as_usize()) {
            block.link_decorators(adjusted_decorators.clone());
        }
    ...
    pub fn block_decorators(&self) -> &BTreeMap<MastNodeId, Arc<Vec<(usize, DecoratorId)>>> {
        &self.block_decorators
    }

    /// Returns a mutable reference to the block decorators mapping.
    pub fn block_decorators_mut(
        &mut self,
    ) -> &mut BTreeMap<MastNodeId, Arc<Vec<(usize, DecoratorId)>>> {
