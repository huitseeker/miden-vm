...
fn render_pvm_constraints_eval(
    ...
    render_masm_constraints_eval(&MasmConstraintsEvalConfig {
        generated_by: "cargo run -p miden-precompiles-prover --release --features registry-tools \
...
fn check(artifacts: &GeneratedArtifacts) -> Result<(), String> {
    ...
    if checked_in != render(artifacts) {
        return Err(format!(
...
fn render(artifacts: &GeneratedArtifacts) -> String {
...
fn write(artifacts: &GeneratedArtifacts) -> io::Result<()> {
    let path = data_path();
    std::fs::write(&path, render(artifacts))
