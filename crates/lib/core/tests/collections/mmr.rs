...
fn test_mmr_tree_with_one_element() -> Result<(), MerkleError> {
    ...
    let stack: Vec<u64> = merkle_root3.iter().map(::as_int).rev().collect();

    // Test case for single element MMR
    let advice_stack: Vec<u64> = merkle_root3.iter().map(::as_int).collect();
    let source = format!(
        "
        use miden::core::collections::mmr
    ...
    let advice_stack: Vec<u64> = merkle_root1
        ...
        .map(::as_int)
        .chain(merkle_root2.iter().map(::as_int))
        .chain(merkle_root3.iter().map(::as_int))
        .collect();
