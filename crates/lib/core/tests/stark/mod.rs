...
fn variable_length_public_inputs(#[case] num_kernel_proc_digests: usize) {
    ...
    let beta =
        QuadFelt::new_complex(Felt::new(auxiliary_rand_values[0]), Felt::new(auxiliary_rand_values[1]));
    let alpha =
        QuadFelt::new_complex(Felt::new(auxiliary_rand_values[2]), Felt::new(auxiliary_rand_values[3]));
