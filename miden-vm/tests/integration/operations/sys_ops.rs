...
fn assert_with_code() {
    ...
    expect_exec_error_matches!(
        ...
        if clk == RowIndex::from(6) && matches!(err.as_ref(), OperationError::FailedAssertion{ err_code, err_msg, .. }
...
fn assert_fail() {
    ...
    expect_exec_error_matches!(
        test,
        ExecutionError::OperationError { clk, err, .. } | ExecutionError::OperationErrorNoContext { clk, err, .. }
        if clk == RowIndex::from(6) && matches!(err.as_ref(), OperationError::FailedAssertion{ err_code, .. }
            if *err_code == ZERO)
    );
}
...
fn assert_eq_fail() {
    ...
    expect_exec_error_matches!(
        test,
        ExecutionError::OperationError { clk, err, .. } | ExecutionError::OperationErrorNoContext { clk, err, .. }
        if clk == RowIndex::from(7) && matches!(err.as_ref(), OperationError::FailedAssertion{ err_code, err_msg, .. }
